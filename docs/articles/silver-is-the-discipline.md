...
File: [`pipelines/airflow/dags/silver_retail_star_schema_dag.py`](../../pipelines/airflow/dags/silver_retail_star_schema_dag.py)
...
File: [`pipelines/airflow/processing/spark/jobs/silver/registry.py`](../../pipelines/airflow/processing/spark/jobs/silver/registry.py)
...
Take [`build_dim_customer_profile`](../../pipelines/airflow/processing/spark/jobs/silver/builders/dim_customer_profile.py). It merges Debezium feeds for users and segments, consolidates simultaneous updates, and runs an SCD2 routine:
...
_Excerpt from `pipelines/airflow/processing/spark/jobs/silver/builders/dim_customer_profile.py`._
...
[`build_fact_order_service`](../../pipelines/airflow/processing/spark/jobs/silver/builders/fact_order_service.py) joins orders, payments, and shipments. Event timestamps pick the right SCD2 slice for each customer and product. The fact emits:
...
[`build_fact_inventory_position`](../../pipelines/airflow/processing/spark/jobs/silver/builders/fact_inventory_position.py) treats each inventory change as a transactional fact. Temporal joins attach the correct warehouse and product version. Rows land with `date_sk` and preserved offsets so supply-chain teams can reconcile history without reruns.

### Customer Engagement

[`build_fact_customer_engagement`](../../pipelines/airflow/processing/spark/jobs/silver/builders/fact_customer_engagement.py) aggregates interactions per user, product, and day. Joins keyed on `last_interaction_ts` honour the customer and product state active when the engagement happened.
...
3. Enable Silver – unpause `silver_retail_service` or run `python pipelines/airflow/processing/spark/jobs/silver_retail_service.py --tables all`. Dimensions build first, facts second, as soon as Bronze publishes fresh Iceberg snapshots.
